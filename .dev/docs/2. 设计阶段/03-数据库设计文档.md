...
| D3 | 软删 + 审计 | 所有业务表 `is_deleted` + `create_time`/`update_time` |
| D4 | 强一致优先 | Seata AT 要求每张业务表必须存在主键索引，禁止跨库事务 |
| D5 | 可加密标记 | 每个敏感字段用 `_enc` 后缀或不写入列名但写入列注释（避免业务代码读取明文） |
| D6 | 时间统一 | 所有时间字段使用 `DATETIME` UTC，存储 UTC，展示由前端按浏览器时区转换 |
...
| 表名 | `t_<module>_<entity>`，全小写下划线 | `t_order`, `t_stock` |
...
| `create_time` | DATETIME NOT NULL | — | UTC |
| `update_time` | DATETIME NOT NULL | — | UTC，行任何 update 触发 |
...
| 字段 | 类型 | 适用表类型 | 含义 |
|---|---|---|---|
| `feature_id` | VARCHAR(32) NULL | 审计 / 流水 / 幂等表 | 关联仓库 `AGENTS.md` 中的 Functional Identifier，如 `USER-ORDER-0001`；用于跨日志 / 文档追溯 |
...
### 5.2 t_auth_user_audit（认证尝试审计，Phase 2 / 待实现）
...
索引：`idx_auth_audit_user(user_id, create_time)`, `idx_auth_audit_ip(client_ip, create_time)`。保留 180 天。
...
`t_merch_audit_log`：商家资质审核日志。每行 `feature_id = PLATFORM-AUTH-0004`（跨文档追溯用，见 [02-API设计文档 §13.1](02-API设计文档.md) 与 [AGENTS.md Functional Identifier Convention](../../../AGENTS.md)）。

---

## 7. platform_db — 平台运营域

### 7.1 t_plat_admin（Phase 2 / 待实现）

平台管理员表，结构同 `t_auth_user` 但 `account_type=3`；本期直接在 `auth_db` 中即可，不必分库。**保留此表是为未来与 `t_auth_user` 解耦预留扩展点。**

### 7.2 t_plat_announcement（Phase 2 / 待实现）
...
### 7.3 t_plat_dispute_case（Phase 2 / 待实现）
...
### 7.4 t_plat_reconciliation_batch（Phase 2 / 待实现）
...
### 7.5 t_plat_reconciliation_diff（Phase 2 / 待实现）
...
### 8.2 t_goods_category（Phase 2 / 待实现）
...
| `publish_at` | DATETIME | 上架时间 |
| 公共字段 | | 见 §3 |

索引：`uk_goods_spu_spu_no(spu_no)`, `idx_goods_spu_merchant_status(merchant_id, status, update_time)`, `idx_goods_spu_category_status(category_id, status)`。
...
### 8.6 t_goods_spu_image / t_goods_spu_attr / t_goods_sku_attr（Phase 2 / 待实现）
...
### 8.8 t_goods_search_sync（Phase 2 / 待实现）
...
### 9.2 t_cart_log（Phase 2 / 待实现）

加车/删车/勾选/清空行为日志，保留 90 天。

### 9.3 t_cart_abandoned_record（Phase 2 / 待实现）
...
- `idx_order_user_status(user_id, status, create_time)`；
- `idx_order_merchant_status(merchant_id, status, create_time)`；
- `idx_order_pay_expire(status, pay_expire_at)`；
- `uk_order_idem(idempotency_key)`。
...
索引：`idx_order_status_log_order(order_id, create_time)`。

### 10.5 t_refund_request / t_refund_log（Phase 2 / 待实现）
...
### 10.8 t_idempotency（Phase 2 / 待实现）
...
### 11.1 t_pay_channel（Phase 2 / 待实现）
...
索引：`uk_pay_trade_no(trade_no)`, `idx_pay_order_no(order_no)`, `idx_pay_user_status(user_id, status, create_time)`, `uk_pay_idem(idempotency_key)`。
...
### 11.5 t_pay_journal（Phase 2 / 待实现）

复式记账流水（每笔金额变动：debit/credit/balance_after）。

### 11.6 t_pay_reconciliation_file（Phase 2 / 待实现）

对账文件元数据（每日下载、解析、对账状态）。

### 11.7 t_pay_idempotency（Phase 2 / 待实现）
...
### 12.3 t_stock_reserve（锁定明细，Phase 2 / 待实现）
...
### 12.5 t_stock_threshold（Phase 2 / 待实现）
...
### 13.1 t_notify_template（Phase 2 / 待实现）
...
### 13.2 t_notify_message（用户/商家站内信）
...
索引：`uk_notify_message_receiver_unread(receiver_role, receiver_id, is_read, create_time)`, `idx_notify_message_biz(biz_ref_type, biz_ref_id)`。

### 13.3 t_notify_push_task（Phase 2 / 待实现）

WebSocket / 短信 / 邮件 推送任务表，状态机：待发→已发→失败→重试中→死信。

### 13.4 t_notify_subscribe / t_notify_subscribe_topic（Phase 2 / 待实现）

订阅主题（运营可定义广播主题）。

### 13.5 t_notify_dead_letter（Phase 2 / 待实现）

死信表（消费失败 N 次后归档）。

---

## 14. 辅助/全局表（Phase 2 / 待实现）
...
| 表 | 索引名 | 列 | 用途 |
|---|---|---|---|
| `t_order` | `idx_order_user_status` | (user_id, status, create_time DESC) | 用户订单列表 |
| `t_order` | `idx_order_merchant_status` | (merchant_id, status, create_time DESC) | 商家订单列表 |
| `t_order` | `idx_order_pay_expire` | (status, pay_expire_at) | 超时扫描 |
| `t_pay_transaction` | `idx_pay_order_no` | (order_no) | 订单查询 |
| `t_stock` | `uk_stock_sku` | (sku_id) UK | 锁定/释放的乐观更新 |
| `t_notify_message` | `uk_notify_message_receiver_unread` | (receiver_role, receiver_id, is_read, create_time DESC) | 红点 + 列表 |
| `t_goods_spu` | `idx_goods_spu_merchant_status` | (merchant_id, status, update_time DESC) | 商家列表 |
| `t_goods_sku` | `uk_goods_sku_sku_no` | (sku_no) UK | 业务单查 |

### 16.3 不能建的索引

- 低区分度字段（性别、状态）不独立建索引，必须组合。
- 不建 `WHERE LEFT(x, n)` / `DATE(create_time)` 类表达式索引。
...
| 表 | 分片键 | 类型 | 路由 | 备注 |
|---|---|---|---|---|
| `t_order` | `user_id` | HASH 64 | mod | 用户维度查 |
| `t_order`（高优写） | `id`（按时间） | RANGE | 月表 | 归档方便 |
| `t_pay_transaction` | `user_id` | HASH 32 | mod | — |
| `t_goods_spu` | `merchant_id` | HASH 32 | mod | 商家隔离 |
| `t_notify_message` | `receiver_id` | HASH 16 | mod | — |
