...
采用 AES-256 字段级加密（ADR-056），密钥托管于 HashiCorp Vault。

| 字段类型 | 加密方式 | 说明 |
|----------|----------|------|
| 手机号 | AES-256，保留前3后4 | 支持模糊查询 |
| 身份证 | AES-256，保留前3后4 | 支持模糊查询 |
| 密码 | BCrypt 单向哈希 | 不可逆 |
| 银行卡号 | SHA-256 哈希 | 不可逆 |
...
- 限流命中返回 `429 Too Many Requests` + `Retry-After` 头 + 统一 RFC 7807。
