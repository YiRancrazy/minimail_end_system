 ...
 * @Description: UploadControllerV1 MockMvc 单元测试，验证用户端分片上传与断点状态查询的路由、参数绑定与 Result 包装。
 * @Version: 1.0
 * @DateTime: 2026/08/12
 **/
class UploadControllerV1Test {
    ...
    void chunk_returns_received_count_when_not_done() throws Exception {
        ...
        mockMvc.perform(multipart("/api/v1/user/upload/chunk")
                .file(file)
    ...
    void status_returns_received_chunks() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/user/upload/status")
                .param("uploadId", "task-1")
                .param("totalChunks", "3"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.code").value("00000"))
